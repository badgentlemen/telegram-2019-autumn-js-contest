...
export const isPeerNotificationMuted = peerNotifySettings => {
    return (
        peerNotifySettings._ == "peerNotifySettings" &&
...
export const getChat = chat_id => {
    return AppstoreInstance.chats.find(chat => chat.id === chat_id)
};

export const getUserString = peerID => {
...
export const getChatString = peerID => {
    ...
    const chat = getChat(peerID);
    if (isChannel(peerID)) {
        return (isMegagroup(peerID) ? "s" : "c") + peerID + "_" + chat.access_hash;
    }
    return "g" + peerID;
};

export const getPeerString = peerID => {
    if (peerID > 0) {
        return getUserString(peerID);
    }

    return getChatString(-peerID);
};


export const isMegagroup = peerID => {
    const chat = getChat(peerID);
    if (!chat) {
        return false;
    }

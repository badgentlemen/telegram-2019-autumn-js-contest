...
class Sidebar {
	constructor(options) {
		...
		this.setLoading = function(loading) {
			if (loading) {
				loadingNode = createElement(
					'div',
					'ui-sidebar__loading-flow',
					node
				);
...
class SidebarBody {
	constructor(container, onItemClick) {
		...
		var floatinButton = createElement(
			'div',
			'ui-dialog__floating-button',
			node
		);
		var pencilImage = createElement(
			'img',
			'ui-image__pencilImage',
			floatinButton
		);
		...
		this.setChats = function(chats) {
			...
			for (let index = 0; index < chats.length; index++) {
				...
				chatListItemNode.addEventListener(
					'click',
					function() {
						onItemClick(chat);
					},
